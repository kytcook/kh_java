...
// 그러니까 추상메소드 앞에는 abstract는 절대로 생략할 수 없다.(컨벤션)
// 추상클래스는 추상메소드와 일반메소드 둘 다를 가져야 하기 때문에 구분을 위한 컨벤션.
// : 왜냐하면 일반메소드와 구분해야 하니깐...
public abstract class Car implements MoveBehavior {
	public int speed = 0;
	public int wheelNum = 0;
	// 추상메소드는 어떤 자동차가 될지를 정할 수가 없어서 추상메소드로 하기로 하였다.
 ...
 * 인터페이스는 오직 추상메소드만 가질 수 있다.
  	: 그러니까 abstract예약어는 생략이 가능하다.
 * 인터페이스는 (일반변수,생성자)를 가질 수 없다.
  	단, 상수는 가능하다.(final 변수 = x;)
 * 추상클래스는 생성자를 갖는다.
  - 추상클래스는 일반메소드와 추상메소드를 모두를 가질 수 있다.
 ...
 * 추상클래스 변수명 = new 구현체클래스(); - 잘하는사람 - 다형성 가능해짐 - 코드수정이 줄어듦
