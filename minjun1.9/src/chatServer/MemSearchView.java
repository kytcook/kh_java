...
public class MemSearchView extends JFrame implements ActionListener,FocusListener,ItemListener{
	...
	String 			  search_item[] = {"검색조건","아이디","이름","전화번호","주소"};
	String			  search	   = null;
	...
	JLabel			  jb_address   = new JLabel("");
	String cols[] = {"아이디","이름","전화번호","주소"}; // header부분
	String data[][] 			   = new String[0][5]; //body부분
	...
	public void dataAdd(List<MemberVO> list) {
		...
		for(int i = 0; i<list.size(); i++) {
			 ...
			 String user_address = mvo.getMem_address();
			 oneRow.add(user_id);
			 oneRow.add(user_name);
			 oneRow.add(user_phone);
			 oneRow.add(user_address);
			 dtm.addRow(oneRow);
		 }	
	}
