...
// 입력할 때는 빈칸을 제공하고 새로입력한 값을 저장해야 한다. - 업무역할
// 수정일때는 기존에 등록된 정보에 대해서 일부를 변경지원한다.
// 입력한 내용 또는 수정할 때 기존 정보를 제공하기 위해서라도 단, 한 건에 대한 상세조회가 선행되어야 한다.
...
public class Main_3 extends JFrame implements ActionListener {
	...
	Dialog_3 dialog = new Dialog_3(); // 자식클래스 변수를 넘긴다. 부모클래스 넘기고 싶으면 super
	// Dialog_3 dialog = null;
	static Main_3 abook = null;
	public Main_3() {
		initDisplay(); // 화면을 그려주는 메소드 호출
	}

	public void initDisplay() { 
   // dialog = new Dialog_3(this); // 자식클래스 변수를 넘긴다. 부모클래스 넘기고 싶으면 super
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (btnInsert == obj) { // 딱 한번 이벤트핸들러가 적용되어야 한다.
			// 자손 클래스에서 부모클래스의 refreshData 메소드를 호출하고 싶어요
			// 그래서 this를 자손클래스의 set메소드의 파라미터로 넘겨 주자
			//dialog.set("입력");
