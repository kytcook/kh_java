...
public class MemberLogin3 extends JFrame implements ActionListener {
	/////////////////////////////////////////////////////
	/* 선언부 */
	/////////////////////////////////////////////////////
	String nickName="";
	...
	public String login(String mem_id, String mem_pw) {
		...
		StringBuilder sql = new StringBuilder(); // String은 원본이 바뀌지 않는다 StringBuilder는 원본이 바뀐다. 메소드 append사용
		sql.append("SELECT                                               ");
	    sql.append("       mem_name                                      ");
	    sql.append("  FROM (                                             ");
	    sql.append("       SELECT                                        ");
		...
		try {
			System.out.println(rs.next());// true - 커서가 다음으로 이동됨 -> if문에서 false나옴
			// 조회 결과는 0이거나 1row 이므로
			if(rs.next()) {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (obj == jbtn_join) {
			...
			String mem_name = login(user_id, user_pw);
			mem_name = login(user_id, user_pw);
			System.out.println("로그인 요청 결과는?? " + mem_name);
			Member3App m2a 	= new Member3App();
			if(mem_name == null ) {
				JOptionPane.showConfirmDialog(this, "회원가입 여부를 확인 하세요");
				return;
