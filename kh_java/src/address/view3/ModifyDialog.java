...
//컴넌트들의 위치를 정해줍니다.
	...
	/**************************************************************************
	 * 
	 * @param title 사용자가 선택한 메뉴에 대한 지식창의 타이틀 값
	 * @param editable 인스턴스화 후 입력 컴포넌트에 쓰기를 허용할건가? 아님 읽기전용으로 할건가를 결정함
	 * 		  입력(수정가능), 수정(수정가능) - true살려놓고 / 상세보기(onlyRead) - false 죽여놓는다
	 * @param vo
	 * @param abook
	 **********************************************************************/
	...
	// 셋팅하는 메쏘드입니다.
	private void setEditable(boolean e) {// 파라미터 boolean에 따라서 결정이 된다. true, flase - 이 안에 메소드를 나눠놨으니까 ㅡㅡ 방법론
  	...
  	private void btnOkayActionPerformed(ActionEvent evt) {
		 ...
		 */
			...
			try {
				...
				// 입력이든 수정이든 반드시 ADrdressCtrl을 경유한다. - 컨벤션(MVC패턴)
				AddressCtrl ctrl = new AddressCtrl();
				// vo안에는 전변이 있다. 어떻게 읽고(getter : getXXX()) 쓰지(setter : setXXX())?
				ctrl.send(vo);// 컨트롤러로 vo를 넘긴다.
			} catch (Exception e) {
