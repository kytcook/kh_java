...
// 접미어에 사용된 VO는 Value Object의 약자이다. 
// 자바의 변수와 오라클의 변수 사이에 매칭해주는 클래스 설계이름이다.
// class안에는 여러개의 전역변수를 선언,호출,부를 수 있다.
public class UserVO {
	// id를 담을 변수 선언
	// 전변의 위변조를 막기 위해서 private으로 선언한 뒤
	// 값을 꺼낼 때는 getter메소드를 호출 하고
	// 값을 담을 때는 (전변에)setter메소드의 파라미터를 통해서 복사된 값이 
	// 전역변수인 mem_id에 저장됨
	...
	// 생성자 역할 : 니가 있어서 내가 RAM(h/w)에 올라 탈 수 있어-재사용하려고
	// 전변을 재사용하기 위해서는 생성자를 반드시 호출 할 것
	// 지금까지는 안해도 문제가 없었잖아요.
	// 배열, collection framwork (자바정석)
	public UserVO() {} 
	// 세 개의 파라미터를 갖는 생성자를 통해서 전역변수의 초기화가 가능하다.
