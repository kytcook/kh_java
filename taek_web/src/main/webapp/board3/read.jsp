	...
	function deleteForm(){
		...
		// 확인시 비번체크는 누가 할까요? html, js, java 중 뉴규 -> js
			// 어디에서?? -> read.jsp(비번), boardDelForm.jsp(확인버튼 눌려져서 서버에 보내지기 전에 체크해야 함)
					// -> read.jsp 
		// read.jsp -> 삭제 -> 모달 -> 비번 입력 받음 -> 확인 버튼 ->action(delete) -> 
...
<!-- 댓글 화면 추가 시작 -->
    <div id="dlg_boardIns" footer="#tb_boardIns" class="easyui-dialog" title="댓글쓰기" data-options="modal:true,closed:true" style="width:600px;height:400px;padding:10px">
        ...
        <form id="f_boardIns" method="get" action="./boardInsert.pj"><!-- 등록 : 목적지가 어디로 갈거니? 새글쓰기or -->
        <!-- 태그안에 자바변수 사용가능한가? 서블릿과 jsp의 역할 -> 가능하다 -->
        <!-- hidden으로 넘어가는 것은 다 get방식이다. 쿼리스트링으로 넘어간다. -->
